...
```{r}
cols <- rep(brewer.pal(6, "Blues")[[3]], 2)
# cols <- rep("grey75", 2)
g <- gather(s, parameter, sample, -response) %>%
  ...
  geom_violin(fill = cols[1], col = NA, adjust = 1.2) +
  coord_flip() +
...
plot_predictions("2", legend_position = c(.87, .23))
ggsave("figs/fig-2.pdf", width = 4.5, height = 2.3)
plot_predictions("3", legend_position = c(.87, .9))
ggsave("figs/fig-3.pdf", width = 4.5, height = 2.3)
plot_predictions("4")
