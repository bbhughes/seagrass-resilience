...
p <- plyr::ldply(fits, function(x) {
  ...
  l <- pp$fit - qnorm(.95) * pp$se.fit
  u <- pp$fit + qnorm(.95) * pp$se.fit # 90% CI
  med <- pp$fit
  data.frame(l, med, u, newdata_this_response)
}) %>% as_data_frame()
...
cols <- c(brewer.pal(4, "Greys")[[3]], brewer.pal(4, "Blues")[[3]])
plot_predictions <- function(figure_id, legend_position = c(.89, .96)) {
  ...
  max_ci <- group_by(pp, response) %>% 
    summarize(max_ci = max(u))
    
  let <- dat %>% left_join(max_ci) %>% 
    group_by(response) %>%
    summarize(label_ordered = label_ordered[1], x = min(ph), y = max(c(value, max_ci))) %>% 
  ...
  geom_ribbon(data = pp_data, aes_string("ph", ymax = "u", ymin = "l", fill = "nutrients_text"), 
    alpha = 0.15, inherit.aes = FALSE) +
  theme_gg() +
  labs(colour = "Nutrients", fill = "Nutrients", y = "Response value", x = "pH") +
  scale_color_manual(values = cols) + scale_fill_manual(values = cols) +
...
plot_predictions("2", legend_position = c(.8, .86))
