...
d3 <- d_mixed %>% select(tank, id, rep, ph1, nutrients1, 
  ...
  filter(nutrients1 != "")
d3$ulva_grazing <- d3$ulva_grazing + 0.01 # TODO fix
d3$epiphyte_grazing <- d3$epiphyte_grazing + 0.01 # TODO fix?
...
d_long <- rbind(d_long, d3)

# join in the raw data with multiple values per tank:
d_long <- rbind(d_long, select(d2, ph, ph_scaled, nutrients, response, value))
