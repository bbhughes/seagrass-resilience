...
# "ulva_grazing",             TRUE,            "gaussian",  "identity", # TODO do not include because "epiphyte" was simply Ulva + diatom?
  "epiphyte_grazing",         TRUE,            "gaussian",  "identity")
...
## Condense these to tank-level means? 
...
Actually, maybe it is cleaner to work directly with the orthogonal polynomials:

```{r}
d_long <- d_long %>% group_by(response) %>%
  mutate(
    ph_scaled = poly(ph_scaled, 2)[,1],
    ph_scaled2 = poly(ph_scaled, 2)[,2]) %>%
  ungroup()
```
...
newdata <- unique(select(d_long, response, ph_scaled, ph_scaled2, ph, nutrients))

newdata <- data.frame(newdata, linear = newdata$ph_scaled, quad = newdata$ph_scaled2)

N <- 100
newdata <- group_by(newdata, response, nutrients) %>%
  arrange(ph) %>%
  do({
...
#     b$b0 + b$b1 * as.numeric(y[["linear"]]) +
#       b$b2 * as.numeric(y[["quad"]]) +
#       b$nutrients * as.numeric(y[["nutrients"]])
#   })
#   l <- apply(pp, 2, quantile, probs = 0.05)
#   u <- apply(pp, 2, quantile, probs = 0.95)
...
p <- plyr::ldply(fits, function(x) {
  ii <<- ii + 1
  newdata_this_response <- as.data.frame(filter(newdata, response == names(fits)[[ii]]))
  newdata_this_response$ph_scaled2 <- newdata_this_response$quad
  pp <- predict(x, newdata = newdata_this_response, se.fit = TRUE)
  l <- pp$fit - qnorm(.9) * pp$se.fit
  u <- pp$fit + qnorm(.9) * pp$se.fit
  med <- pp$fit
  data.frame(l, med, u, newdata_this_response)
}) %>% as_data_frame()
