...
cis <- mutate(cis, parameter = factor(parameter, levels = c("linear", "quadratic", "nutrients"),
    labels = c("Linear", "Quadratic", "Nutrients")))
cis <- mutate(cis, label_ordered = reorder(label, -as.numeric(as.factor(figure_panel))))
...
g <- gather(s, parameter, sample, -response) %>%
  ...
  mutate(parameter = factor(parameter, levels = c("linear", "quadratic", "nutrients"),
    labels = c("Linear", "Quadratic", "Nutrients"))) %>%
  mutate(label_ordered = reorder(label, -as.numeric(as.factor(figure_panel)))) %>% 
  ...
  geom_segment(data = cis, aes(x = label_ordered, y = l, xend = label, yend = u),
    lwd = 0.4, col = "grey20") +
  geom_vline(xintercept = c(4.5, 8.5, 12.5), col = "grey50", lty = 2)
g
...
p <- plyr::ldply(fits, function(x) {
  ...
  l <- pp$fit - qnorm(.975) * pp$se.fit
  u <- pp$fit + qnorm(.975) * pp$se.fit
  med <- pp$fit
  data.frame(l, med, u, newdata_this_response)
}) %>% as_data_frame()
...
pp$label_wrap = stringr::str_wrap(pp$label, width = 30)
d_long_plot$label_wrap = stringr::str_wrap(d_long_plot$label, width = 30)

cols <- c(brewer.pal(4, "Greys")[[3]], brewer.pal(4, "Blues")[[3]])
plot_predictions <- function(figure_id, legend_position = c(.89, .93)) {
  dat <- filter(d_long_plot, substr(figure_panel, 0, 1) == figure_id)
  point_data <-filter(pp, substr(figure_panel, 0, 1) == figure_id)
  g <- ggplot(dat, aes_string("ph", "value", colour = "nutrients_text")) + 
  geom_point(alpha = 0.3) +
  facet_wrap(~label_wrap, scales = "free_y") +
  geom_line(data = point_data, aes_string("ph", y = "med"), lwd = 1) +
  geom_ribbon(data = point_data, aes_string("ph", ymax = "u", ymin = "l", fill = "nutrients_text"), 
    alpha = 0.15, inherit.aes = FALSE) +
  theme_gg() +
  labs(colour = "Nutrients", fill = "Nutrients", y = "Response value", x = "pH") +
  scale_color_manual(values = cols) + scale_fill_manual(values = cols) +
...
plot_predictions("2", legend_position = c(.1, .86))
ggsave("figs/fig-2.pdf", width = 7.2, height = 2.8)
plot_predictions("3")
ggsave("figs/fig-3.pdf", width = 6, height = 5.5)
plot_predictions("4")
ggsave("figs/fig-4.pdf", width = 6, height = 5.5)
plot_predictions("S")
ggsave("figs/fig-S1.pdf", width = 6, height = 5.5)
```
