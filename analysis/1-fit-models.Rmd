...
# there was x-fold higher diatom recruitment when nutrients were added:
x <- as.data.frame(as.matrix(fits[["diatom_grazing"]]))[,"nutrients"]
round(exp(quantile(x, probs = c(0.025, 0.5, 0.975))), 1)

# Elevated nutrient conditions were associated with smaller changes in mean seahare biomass 
x <- as.data.frame(as.matrix(fits[["change_seahare_mass_g_fw"]]))[,"nutrients"]
round((quantile(x, probs = c(0.025, 0.5, 0.975))), 1)
