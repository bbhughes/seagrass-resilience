...
p <- plyr::ldply(fits, function(x) {
  ii <<- ii + 1
  newdata_this_response <- as.data.frame(filter(newdata, response == names(fits)[[ii]]))
  pp <- posterior_linpred(x, newdata = newdata_this_response)
  l <- apply(pp, 2, quantile, probs = 0.1)
  u <- apply(pp, 2, quantile, probs = 0.9)
...
plot_predictions("4", legend_position = c(.3, .1))
...
plot_predictions("S", whole_numbers = TRUE, legend_position = c(.25, .93))
...
message("The flight shoot length was X% longer at pH 7.45 than 7.86")
...
message("Should mortality was x-fold higher when nutrients were added:")
...
jackknife <- function(dat) {
  fits_jk <- plyr::ddply(dat, "response", function(x) {
    ...
    m <- stan_glm(f, data = x, iter = 1000, chains = 4, family = fam,
      prior = normal(0, 2),
      prior_intercept = normal(0, 10),
      prior_aux = prior_aux)
    ...
    pp <- posterior_linpred(m, newdata = newdata_this_response)
    med <- apply(pp, 2, median)
...
jk <- plyr::ldply(unique(d_long$tank), 
  function(i) {
    data.frame(jackknife(filter(d_long, tank != i)), tank_removed = i)})
